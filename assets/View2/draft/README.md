...
[![TurboFEM demo preview](./assets/turbofem-preview.gif)](./assets/turbofem-demo.mp4)

*Click the preview to watch the full ~2-minute walkthrough — from "Let's Validate TurboFEM" to a solved, validated 1D beam.*

**[▶ Watch the full demo (turbofem-demo.mp4)](./assets/turbofem-demo.mp4)**

<!--
  NOTE FOR PUBLISHING ON GITHUB:
  GitHub's README renderer plays GIFs inline but does NOT play .mp4
  files inline. The GIF above already works as-is once this file is
  in the repo. For the full mp4 to also play inline (not just via
  download link), drag-and-drop `turbofem-demo.mp4` into a new GitHub
  Issue or PR comment box — GitHub will upload it to their CDN and
  give you a `https://github.com/user-attachments/...` link. Replace
  the relative link above with that CDN link and it will play inline
  for anyone visiting the repo.
-->
...
- 🎯 **Solid linear FEA accuracy**, powered by [CalculiX](http://www.calculix.de/), a proven, industry-used open-source solver — so speed doesn't come at the cost of trustworthy results.
...
| 🔩 **Solver integration** | Built on top of CalculiX for validated linear mechanical analysis |
...
The short walkthrough above validates a classic **1D beam** case end-to-end:

1. Define the geometry and cross-section
2. Assign material and boundary conditions
3. Run the solver (CalculiX under the hood)
